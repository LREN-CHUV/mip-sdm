...
import { ProgressLog, spawnAndWatch, CodeTransform, AutofixRegistration, LoggingProgressLog } from "@atomist/sdm";
    ...
    await project.findFile(PreCommitConfig).then(async file => {
      ...
      if (content.indexOf("repos:") < 0 || content.indexOf("sha:") >= 0) {
        if (isLocalProject(project)) {
          executePreCommitMigrateConfig(project as LocalProject, progressLog)
        }
      }
      ...
      config.repos.forEach(repo => {
        if (repo.repo == "git://github.com/pre-commit/pre-commit-hooks") {
          ...
          if (!hasHook("check-yaml", hooks) && isLocalProject(project) && hasYamlFiles(project as LocalProject)) {
            hooks.push({"id": "check-yaml"});
          }
          if (!hasHook("check-json", hooks) && isLocalProject(project) && hasJsonFiles(project as LocalProject)) {
            hooks.push({"id": "check-json"});
          }
          if (!hasHook("pretty-format-json", hooks) && isLocalProject(project) && hasJsonFiles(project as LocalProject)) {
            hooks.push({"id": "pretty-format-json", "args": ["--autofix"], "exclude": "slack.json"});
          }
        }
        
      });

      await file.setContent(updateYamlDocument({ repos: config.repos }, content));
    });

    if (isLocalProject(project)) {
      executePreCommitOnProject(project as LocalProject, progressLog);
    }
  }
  return project;
};
...
export async function hasJsonFiles(project: LocalProject): Promise<boolean> {
  return await fileExists(project, "**/*.json", f => f.name != "slack.json");
}

export async function hasYamlFiles(project: LocalProject): Promise<boolean> {
  return await fileExists(project, "**/*.yml") || fileExists(project, "**/*.yaml");
}
